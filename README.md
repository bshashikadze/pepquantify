...
Important: set experimental_library to FALSE in case of library-free search with MBR enabled and TRUE in case when empirical library is used (e.g. pre-fractionation, GPF and etc.). if contaminants fasta file was provided during identification the same fasta file should be placed in the working directory and remove_contaminants should be set to TRUE (default is false). Other parameters could be left as default
...
* remove_contaminants: if during DIA-NN search contaminants fasta file has been used, remove contaminants can be set to TRUE (directory should contain the same contaminats fasta file). false otherwise
