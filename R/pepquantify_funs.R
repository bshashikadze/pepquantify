...
#' @param n_element_peptide peptide data is nth element
...
#' @param n_condition_1 minimum number of the valid values in the first condition (this value should be minimum two but default pepquant value is three)
#' @param n_condition_2 minimum number of the valid values in the second condition (this value should be minimum two but default pepquant value is three)
#' @param min_pep minimum number of peptides
...
#' @param imputation if true imputation is performed if false not
...
pep_data <-  pepquantify::preppeptide(data, n_element_peptide = n_element_peptide, condition1 = condition1, condition2 = condition2, n_condition_1 = n_condition_1, n_condition_2 = n_condition_2, min_pep = min_pep)

  if (imputation == TRUE) {
pepquantify::peptimpute(pep_data, downshift = downshift, width = width, n_ko_like = n_ko_like, fraction_valid = fraction_valid,
                           second_condition = second_condition, seed = seed)

  }
locations_last <- pepquantify::findlastpepfile()
}
